...
def extract_subjects(query: str):
    ...
    parts = re.split(r"\bvs\b|\band\b|,", query)
...
def parse_subject(subject: str):
    ...
    for w in words[1:]:
        # If it's a known descriptor, it's a variant
        if w in DESCRIPTOR_TERMS:
            variant.append(w)
        # If it contains a digit, it's likely the model ID (e.g., '15', 's23', '8t')
        elif re.search(r'\d', w):
            model_id = w
        # Otherwise it's part of the core name (e.g., 'pixel', 'galaxy')
        else:
            core.append(w)
    ...
    return {
        "brand": brand,
        "core": core,
        "iteration": model_id, # Mapping to 'iteration' for legacy compatibility
        "variant": " ".join(variant) if variant else None
    }
