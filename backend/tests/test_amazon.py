from backend.extractors.site_extractors.amazon import parse_html


def load_amazon_fixture() -> tuple[str, str]:
    fixture_path = "context/amazon_india.txt"
    with open(fixture_path, "r", encoding="utf-8") as f:
        raw = f.read()

    url_marker = "URL:"
    html_marker = "HTML:"

    url_start = raw.find(url_marker)
    html_start = raw.find(html_marker)

    if url_start == -1 or html_start == -1:
        raise RuntimeError("Amazon fixture is missing URL or HTML markers.")

    url_line = raw[url_start + len(url_marker):].splitlines()[0].strip()
    html = raw[html_start + len(html_marker):].strip()
    return url_line, html


def main():
    url, html = load_amazon_fixture()
    data = parse_html(html, url)

    if not data:
        raise RuntimeError("Amazon extractor returned no data.")

    print("Title:", data["title"])
    print("Table count:", len(data.get("tables", [])))
    print("Opinion count:", len(data.get("opinions", [])))

    print("\n--- SAMPLE TABLE ROWS ---")
    for row in data.get("tables", [])[:12]:
        extra = ""
        if row.get("metadata", {}).get("kind"):
            extra = f" ({row['metadata']['kind']})"
        print(f"[{row['aspect']}] {row['value']}{extra}")

    print("\n--- SAMPLE OPINIONS ---")
    for row in data.get("opinions", [])[:6]:
        print(f"[{row['source']}] {row['text']}")


if __name__ == "__main__":
    main()
